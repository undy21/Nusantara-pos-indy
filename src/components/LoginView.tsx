    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4 sm:p-6 md:p-8" id="login_container">
      ...
      <div className="w-full max-w-md bg-slate-950/80 backdrop-blur-md rounded-2xl border border-slate-800 shadow-2xl p-6 sm:p-8 z-10" id="login_card">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-500 to-sky-400 p-2 text-slate-950 mb-4 shadow-lg shadow-indigo-500/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-100 placeholder-slate-500 text-xs focus:outline-none focus:border-indigo-500 transition-colors"
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-100 placeholder-slate-500 text-xs focus:outline-none focus:border-indigo-500 transition-colors"
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-gradient-to-r from-indigo-600 to-sky-600 hover:from-indigo-500 hover:to-sky-500 text-white font-extrabold text-xs py-3 px-4 rounded-xl cursor-pointer shadow-md shadow-indigo-600/10 hover:shadow-indigo-600/20 active:scale-98 transition-all flex items-center justify-center gap-2 mt-2"
            id="login_submit_btn"
          >
          ...
          <div className="grid grid-cols-2 gap-2" id="demo_accounts_grid">
            {userList.length === 0 ? (
              ...
              [
                ...
                >
                  <p className="text-[10px] font-bold text-slate-300 truncate group-hover:text-indigo-400">{fallbackUsr.name}</p>
              ...
              userList.map((usr) => (
                <button
                  ...
                  className="p-2 border border-slate-800/80 bg-slate-900/60 hover:bg-slate-800 rounded-lg text-left transition-all hover:border-slate-700 cursor-pointer group"
                >
                  <p className="text-[10px] font-bold text-slate-300 truncate group-hover:text-indigo-400">{usr.name}</p>
                  <div className="flex justify-between items-center text-[9px] text-slate-500 mt-1">
                    <span>{usr.username}</span>
                    <span className="bg-slate-800 px-1 rounded text-[8px] font-bold text-indigo-400 truncate tracking-tight">{usr.role}</span>
                  </div>
                </button>
              ))
            )}
