      ...
      <div className="flex-1 flex flex-col p-5 space-y-4 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 shrink-0">
          ...
          <form onSubmit={handleBarcodeSubmit} className="relative bg-white border border-slate-200 rounded-xl px-3.5 py-2 flex items-center shadow-sm">
            <Barcode className="w-5 h-5 text-indigo-600 shrink-0 mr-2" />
            <input
              ref={barcodeInputRef}
              type="text"
            ...
            filteredProducts.map((p) => {
              ...
              return (
                <div
                  key={p.sku}
                  onClick={() => addToCart(p)}
                  className={`bg-white border rounded-2xl p-3 flex flex-col justify-between hover:border-indigo-400 cursor-pointer shadow-sm hover:shadow-md transition-all relative overflow-hidden group ${p.stock <= 0 ? 'opacity-65 select-none' : ''}`}
                >
                  {/* Stock flag */}
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            {selectedCustomer ? (
              <div className="min-w-0">
                ...
                <span className="text-[9px] text-indigo-600 font-bold uppercase tracking-wider">{selectedCustomer.memberRank} Member • {selectedCustomer.point} pts</span>
              </div>
            ) : (
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50 space-y-3.5 shrink-0">
          ...
          <form onSubmit={handleApplyVoucher} className="flex gap-2">
            <div className="relative flex-grow bg-white border border-slate-200 rounded-xl px-2.5 py-1.5 flex items-center shadow-xs">
              <Tag className="w-3.5 h-3.5 text-indigo-600 shrink-0 mr-1.5" />
              <input
                type="text"
            ...
            <button
              type="submit"
              className="px-3.5 py-1.5 rounded-xl text-xs font-bold bg-indigo-600 hover:bg-indigo-750 text-white transition-colors cursor-pointer"
            >
              Klaim
            </button>
          </form>
          ...
          {appliedVoucher && (
            <div className="text-[10px] text-indigo-600 bg-indigo-50 border border-indigo-100 px-2.5 py-1.5 rounded-lg flex items-center justify-between font-bold">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl w-full max-w-sm p-6 space-y-5 animate-in fade-in-50 zoom-in-95 shadow-2xl border border-slate-100">
            ...
            {paymentMethod === 'CASH' && (
              <div className="space-y-3.5">
                ...
                {cashReceived >= calculatedTotals.finalAmount && (
                  <div className="p-3 bg-indigo-50 border border-indigo-100 text-indigo-600 font-bold rounded-xl flex items-center justify-between text-xs">
                    <span>Kembalian</span>
            ...
            {paymentMethod === 'MEMBER_POINTS' && selectedCustomer && (
              <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-2xl space-y-2 text-xs text-slate-600 font-medium">
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-indigo-600 font-mono">{selectedCustomer.point} Pts</span>
                </div>
                <div className="flex justify-between">
                ...
                {selectedCustomer.point * 10 < calculatedTotals.finalAmount ? (
                  ...
                  <p className="text-[10px] text-indigo-600 font-bold mt-2 text-center bg-white px-2 py-1 rounded border border-indigo-100">
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-700/60 shadow-2xl rounded-3xl w-full max-w-sm flex flex-col justify-between overflow-hidden animate-in zoom-in-95">
            ...
            <div className="px-5 py-4 border-b border-slate-800 flex items-center justify-between text-white bg-slate-950/40">
              <span className="text-xs font-black uppercase tracking-wider text-indigo-400">Transaksi Sukses</span>
            ...
            <div className="p-4 flex-1 overflow-y-auto max-h-[420px] bg-white text-slate-800 flex flex-col items-center">
              ...
              >
                {/* Brand Header */}
                <h3 className="text-sm font-black text-slate-950 uppercase tracking-widest font-sans">NUSANTARA POS</h3>
            ...
            <div className="p-4 bg-slate-950/90 border-t border-slate-800 flex items-center justify-between gap-3 shrink-0">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl text-xs font-bold bg-indigo-600 hover:bg-indigo-750 text-white flex items-center justify-center gap-1 cursor-pointer"
              >
